...
import {
  AuditCommandHandlers,
  BusinessEventParams,
  SecurityEventParams,
} from "../commands/audit-command.handlers";
import {
  AuditAction,
  AuditEventType,
  AuditTrailStatus,
...
export class AuditEventHandlers {
  constructor(private readonly auditCommandHandlers: AuditCommandHandlers) {}
  ...
  async handleComplaintEvent(event: DomainEvent): Promise<void> {
    try {
      ...
      logger.error(
        "Failed to handle complaint event for audit",
        error as Error,
        {
          eventId: event.eventId,
          eventType: event.eventType,
  ...
  async handleTransactionEvent(event: DomainEvent): Promise<void> {
    try {
      ...
      logger.error(
        "Failed to handle transaction event for audit",
        error as Error,
        {
          eventId: event.eventId,
          eventType: event.eventType,
  ...
  async handleSecurityEvent(event: DomainEvent): Promise<void> {
    try {
      ...
      logger.error(
        "Failed to handle security event for audit",
        error as Error,
        {
          eventId: event.eventId,
          eventType: event.eventType,
  ...
  /**
   * Handle complaint saga events specifically
   */
  async handleComplaintSagaEvent(event: any): Promise<void> {
    try {
      logger.info("Processing complaint saga event for audit", {
        eventId: event.eventId,
        eventType: event.eventType,
        aggregateId: event.aggregateId,
      ...
      const action = this.mapComplaintSagaEventToAction(event.eventType);
      if (!action) {
        return;
      }

      const params: BusinessEventParams = {
        entityType: "ComplaintSaga",
        entityId: event.aggregateId,
        action,
        serviceName: "complaint-saga",
        correlationId: event.correlationId,
        details: {
          eventType: event.eventType,
          eventData: event.eventData,
          version: event.version,
          sagaStep: this.extractSagaStep(event.eventType),
          complaintId: event.eventData.complaintId,
        },
        userId: event.eventData.customerId,
        causationId: event.causationId,
      };

      await this.auditCommandHandlers.logBusinessEvent(params);

      // Create or update saga trail for complaint saga
      if (event.eventType === "COMPLAINT_SAGA_INITIATED") {
        await this.auditCommandHandlers.createAuditTrail({
          entityType: "ComplaintSaga",
          entityId: event.aggregateId,
          processName: "ComplaintHandling",
          correlationId: event.correlationId,
          metadata: {
            complaintId: event.eventData.complaintId,
            customerId: event.eventData.customerId,
            complaintType: event.eventData.complaintType,
            priority: event.eventData.priority,
            initiatedAt: event.timestamp,
          },
        });
      } else if (
        event.eventType === "COMPLAINT_SAGA_COMPLETED" ||
        event.eventType === "COMPLAINT_SAGA_FAILED"
      ) {
        // Update trail status
        const trail = await this.auditCommandHandlers[
          ...
          await this.auditCommandHandlers.updateAuditTrail({
            ...
            status:
              event.eventType === "COMPLAINT_SAGA_COMPLETED"
                ...
                : AuditTrailStatus.FAILED,
            endTime: new Date(),
            metadata: {
              ...trail.metadata,
              finalStatus: event.eventData.finalStatus,
      ...
      logger.info("Complaint saga event audit log created", {
        eventId: event.eventId,
        action,
      ...
      logger.error(
        "Failed to handle complaint saga event for audit",
        error as Error,
        {
          eventId: event.eventId,
          eventType: event.eventType,
  ...
  private async updateAuditTrail(event: DomainEvent): Promise<void> {
    ...
    if (this.isProcessEndEvent(event)) {
      const trail = await this.auditCommandHandlers[
        "auditTrailRepository"
      ].findByCorrelationId(event.metadata.correlationId);

      if (trail) {
        await this.auditCommandHandlers.updateAuditTrail({
          trailId: trail.trailId,
          status: this.isSuccessEvent(event)
            ? AuditTrailStatus.COMPLETED
            : AuditTrailStatus.FAILED,
          endTime: new Date(),
        });
  ...
  private mapComplaintSagaEventToAction(eventType: string): AuditAction | null {
    const mapping: Record<string, AuditAction> = {
      COMPLAINT_SAGA_INITIATED: AuditAction.SAGA_START,
      CUSTOMER_VALIDATION_STARTED: AuditAction.SAGA_STEP,
      CUSTOMER_VALIDATION_COMPLETED: AuditAction.SAGA_STEP,
      CUSTOMER_VALIDATION_FAILED: AuditAction.SAGA_STEP,
      ORDER_VERIFICATION_STARTED: AuditAction.SAGA_STEP,
      ORDER_VERIFICATION_COMPLETED: AuditAction.SAGA_STEP,
      ORDER_VERIFICATION_FAILED: AuditAction.SAGA_STEP,
      RESOLUTION_PROCESSING_STARTED: AuditAction.SAGA_STEP,
      REFUND_PROCESSING_COMPLETED: AuditAction.SAGA_STEP,
      REPLACEMENT_PROCESSING_COMPLETED: AuditAction.SAGA_STEP,
      STORE_CREDIT_PROCESSING_COMPLETED: AuditAction.SAGA_STEP,
      RESOLUTION_PROCESSING_FAILED: AuditAction.SAGA_STEP,
      COMPLAINT_SAGA_COMPLETED: AuditAction.SAGA_COMPLETE,
      COMPENSATION_INITIATED: AuditAction.SAGA_COMPENSATE,
      REFUND_COMPENSATION_COMPLETED: AuditAction.SAGA_COMPENSATE,
      REPLACEMENT_COMPENSATION_COMPLETED: AuditAction.SAGA_COMPENSATE,
      CREDIT_COMPENSATION_COMPLETED: AuditAction.SAGA_COMPENSATE,
      COMPENSATION_FAILED: AuditAction.SAGA_COMPENSATE,
      COMPLAINT_SAGA_FAILED: AuditAction.SAGA_COMPLETE,
    };
    return mapping[eventType] || null;
  }

  private extractSagaStep(eventType: string): string {
    if (eventType.includes("CUSTOMER_VALIDATION")) return "CUSTOMER_VALIDATION";
    if (eventType.includes("ORDER_VERIFICATION")) return "ORDER_VERIFICATION";
    if (
      eventType.includes("RESOLUTION_PROCESSING") ||
      eventType.includes("REFUND_PROCESSING") ||
      eventType.includes("REPLACEMENT_PROCESSING") ||
      eventType.includes("STORE_CREDIT_PROCESSING")
    ) {
      return "RESOLUTION_PROCESSING";
    }
    if (eventType.includes("COMPENSATION")) return "COMPENSATION";
    if (eventType.includes("SAGA_INITIATED")) return "INITIATION";
    if (
      eventType.includes("SAGA_COMPLETED") ||
      eventType.includes("SAGA_FAILED")
    )
      return "COMPLETION";
    return "UNKNOWN";
  }
  ...
  private determineAuditAction(event: DomainEvent): AuditAction {
    ...
    if (
      ...
      event.eventType.includes("REMOVED")
    )
  ...
  private isSuccessEvent(event: DomainEvent): boolean {
    ...
    return successEvents.some((successEvent) =>
      event.eventType.includes(successEvent)
    );
  }
}
